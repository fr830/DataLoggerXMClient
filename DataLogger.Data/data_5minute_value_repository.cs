...
namespace DataLogger.Data
{
    public class data_5minute_value_repository : NpgsqlDBConnection
    {
        ...
        public int updateCustom(ref data_value obj, List<string> custom_param_list) {
            ...
            using (NpgsqlDBConnection db = new NpgsqlDBConnection())
            {
                try
                {
                    if (db.open_connection())
                    {
                        ...
                        string sql_command = @"UPDATE data_5minute_values 
                                               SET  {custom_param}
                                               where id = :id
                                                ";
                        ...
                        {
                            foreach (string customparam in custom_param_list)
                            {
                                string customparamStatus = customparam + "_status";
                                custom_param = custom_param + customparam + "=:" + customparam + ","
                                               + customparamStatus + "=:" + customparamStatus + ","
                                               ;

                            }
                            //custom_param = " , " + string.Join(",", custom_param_list);
                        }
                        custom_param = custom_param.TrimEnd(custom_param[custom_param.Length - 1]);
                        ...
                        using (NpgsqlCommand cmd = db._conn.CreateCommand())
                        {
                            ...
                            foreach (string customparam in custom_param_list)
                            {
                                string parameterName = customparam; //VD : var1
                                string _parameterName = ":" + customparam;
                                string parameterStatusName = customparam + "_status"; //VD : var1_status
                                string _parameterStatusName = ":" + customparam + "_status";

                                Type parameterNameType = typeof(data_value);
                                PropertyInfo prop = parameterNameType.GetProperty(parameterName);
                                double value = (double)prop.GetValue(obj); //VD : lay display_var = data.var1
                                cmd.Parameters.Add(_parameterName, NpgsqlTypes.NpgsqlDbType.Double).Value = value;

                                Type parameterNameStatusType = typeof(data_value);
                                PropertyInfo propStatus = parameterNameType.GetProperty(parameterStatusName);
                                int status = (int)propStatus.GetValue(obj); //VD : lay display_var = data.var1
                                cmd.Parameters.Add(_parameterStatusName, NpgsqlTypes.NpgsqlDbType.Integer).Value = status;
                            }
        ...
        public int get_id_by_dt(string dt, string hour, string minute)
        {
            using (NpgsqlDBConnection db = new NpgsqlDBConnection())
            {
                try
                {
                    ...
                    if (db.open_connection())
                    {
                        string sql_command = "SELECT id FROM data_5minute_values WHERE DATE(stored_date) = " + "\'" + dt + "\'" + " and stored_hour = " + "\'" + hour + "\'" + " and stored_minute = " + "\'" + minute + "\'";
                        //sql_command += " LIMIT 1";
                        using (NpgsqlCommand cmd = db._conn.CreateCommand())
                        {
                            cmd.CommandText = sql_command;
